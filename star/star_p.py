...
count = {'sinbal':0, 'ticket':0, 'daily':0}
no_coin = [False, False]
...
Stages = gam.set_stages(
	...
	M='map', M_A='map_a', R='raid', R_A='raid_a', 
	Q='quest', Q_F='quest_f', Q_B='quest_bag', Q_N='quest_n', Q_A='quest_a', 
	MI='migung', MI_A='migung_a', A='arena', A_A='arena_a', 
	P='play', P_A='play_a', P_R='play_r', P_RB='play_daily_boss',
	B='bag', B_L2='bag_l2', B_L='bag_l', B_D='bag_detail', B_S='bag_s', B_SP='bag_sp', B_P1='bag_p1', B_P2='bag_p2',
	BO='bonus', D='daily')
...
def check_bonus():
	# gam.touch(910, 970, '출석 보너스')
	gam.touch(990, 970, '출석 보너스')
	MonkeyRunner.sleep(2.0)
	# gam.touch(890, 720, '확인')
...
def empty_bag():
	...
	while True:
		...
		if stage is Stages.B:
			# if not gam.check_stage(shot, Stages.B_L2): 
...
# -15035905:A, -14364067:B/P, -6408193:S, -5863090:N, -2941172:SSS, -27357:SS, -1:C
			...
			if -10000000 > raw_pixel_int or -10000 < raw_pixel_int:
				...
				gam.touch(1798, 290-10 + 190 * l, '장비창:' + `l`)
			current_line -= 1
			...
			if gam.check_stage(shot, Stages.B_P1) or gam.check_stage(shot, Stages.B_P2):
				# gam.touch(1085, 730, '물약 판매')
				gam.touch(1180, 730, '물약 판매')
			else:
				# gam.touch(730, 730, '취소')
				gam.touch(794, 730, '취소')
			# gam.touch(1624, 111, '장비창 나가기')
...
def check_home(stage, msg):
	if stage is Stages.H:
		# gam.touch(1700, 800, msg+':레이드 초대 취소')
		gam.touch(1850, 800, msg+':레이드 초대 취소')
		return 1
	elif stage is Stages.H_AD: 
		# gam.touch(700, 975, msg+':광고 취소')
...
def run_quest(is_infinity):
	...
	while True:
		...
		if stage is Stages.Q:
			...
			MonkeyRunner.sleep(1.0)
			# gam.touch(1000, 720, dm+'입장 하기-친구 없을시')
	...
	while True:
		...
		if home_checked > 0:
			if home_checked is 1:
		   	...
		   	# gam.touch(622, 943, msg+':전설선택')

		   	#코모도 페어리
		   	# gam.device.drag((900, 0), (900, 970), 0.1, 10) #맵 이동
		   	# gam.device.drag((1630, 660), (0, 660), 0.1, 10)
		   	# gam.touch(1440, 300, msg+':코모도 페어리')
		   	#스톰 타일런트
		   	# gam.device.drag((900, 0), (900, 970), 0.1, 10) #맵 이동
		   	# gam.device.drag((0, 660), (1630, 660), 0.1, 10)
		   	# gam.touch(280, 860, msg+':스톰 타일런트')
		   	#정글 퓨리
		   	# gam.device.drag((900, 970), (900, 0), 0.1, 10) #맵 이동
		   	# gam.device.drag((1630, 600), (0, 600), 0.1, 10)
		   	# gam.touch(1230, 730, msg+':정글 퓨리')

		   	# 코모도 페어리
		   	# gam.device.drag((979, 0), (979, 970), 0.1, 10) #맵 이동
		   	# gam.device.drag((1774, 660), (0, 660), 0.1, 10)
		   	# gam.touch(1567, 300, msg+':코모도 페어리')
		   	# 스톰 타일런트
		   	# gam.device.drag((979, 0), (979, 970), 0.1, 10) #맵 이동
		   	# gam.device.drag((0, 660), (1774, 660), 0.1, 10)
		   	# gam.touch(304, 860, msg+':스톰 타일런트')
		   	# 정글 퓨리
		   	# gam.device.drag((979, 970), (979, 0), 0.1, 10) #맵 이동
		   	# gam.device.drag((1774, 600), (0, 600), 0.1, 10)
		   	# gam.touch(1338, 730, msg+':정글 퓨리')

			#블랙 마운틴
		   	# gam.device.drag((900, 5), (900, 970), 0.1, 10)
		   	# gam.device.drag((0, 300), (1630, 300), 0.1, 10)
		   	# gam.touch(1247, 422, msg+':블랙 마운틴')
		   	gam.device.drag((979, 5), (979, 970), 0.1, 10)
		   	gam.device.drag((0, 300), (1774, 300), 0.1, 10)
		   	gam.touch(1357, 422, msg+':블랙 마운틴')
...
def run_daily(is_infinity):
	mode = 'daily:'
	is_played = False
	# daily_points = [(600,  350), (800,  350), (1000, 350), (1000, 600), (1200, 600), (1400, 600), (600,  600)]
	...
	while True:
		...
		if stage is Stages.M:
		   	...
		   	# gam.device.drag((1630, 660), (0, 660), 0.1, 10)
		   	gam.device.drag((1774, 660), (0, 660), 0.1, 10)
		   	# gam.touch(490, 550, msg+':요일 특별 던전')
			...
			gam.touch(p[0], p[1], msg+'요일 던전 선택') 
				...
				no_coin[0] = True
				# gam.touch(966, 756, msg+'신발 부족 확인')
				gam.touch(1051, 756, msg+'신발 부족 확인')
				if not is_infinity: 
					break
		elif stage is Stages.Q_F:
			# gam.touch(1355, 227, msg+'친구 선택')
			gam.touch(1474, 227, msg+'친구 선택')
			# gam.touch(1000, 994, msg+'입장 하기')
			gam.touch(1088, 994, msg+'입장 하기')
			MonkeyRunner.sleep(1.0)
			# gam.touch(1000, 720, msg+'입장 하기-친구 없을시')
			...
			# gam.touch(1644, 5, msg+':탐험실패')
			gam.touch(1789, 5, msg+':탐험실패')
			# gam.touch(1200, 994, msg+'탐험 성공/실패 : 확인, 동료맞이')
...
if mode == 'QR':
	while True:
		...
		MonkeyRunner.sleep(1.0)
		# gam.touch(720, 720, 'QR:레이드 초대 거절 확인')
